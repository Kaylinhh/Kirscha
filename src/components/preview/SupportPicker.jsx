// src/components/preview/SupportPicker.jsx
import { useState } from 'react'
import { supportCategories } from '../../data/data'

function SupportPicker({ artworkImage, onBack, initialCategoryId = null, initialItemId = null }) {
  const [activeCategory, setActiveCategory] = useState(initialCategoryId)

  const preselectedItem =
    initialCategoryId && initialItemId
      ? supportCategories
  ...
  const [previewImage, setPreviewImage] = useState(
    preselectedItem ? preselectedItem.mockup : artworkImage
  )
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-0 mt-10 -mx-30">
      <img src={previewImage} alt="" className="w-full sm:w-1/2 object-cover" />
      ...
      <div className="w-full sm:w-1/2 bg-kirscha-green-700 p-10">
        ...
        <button onClick={onBack} className="font-body text-kirscha-green-100 underline mb-6 block">
          ← Retour
        </button>
